            ...
            <div class="card">
                <div class="card-body">
                    <div class="border p-4 rounded">
                        ...
                        {{-- form --}}
                        <form method="POST" action="{{ route('shipping.update', $shipping_address->id) }}">
                            @CSRF
                            @method('PUT')
                                ...
                                <div class="card">
                                        ...
                                        <select name="order_id" class="form-control" required>
                                            @foreach ($orders as $order)
                                                <option value="{{ $order->id }}"
                                                    {{ $order->id == $shipping_address->order_id ? 'selected' : '' }}>
                                                    Order #{{ $order->id }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <input class="form-control" type="text" placeholder="country name"
                                            name="shipping_address_country" value="{{ $shipping_address->country }}"
                                            required>
                                    </div>
